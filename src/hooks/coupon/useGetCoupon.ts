...
export function useGetMainCoupon(orderBy: string) {
  return useQuery(["getMainCoupon"], () => getMainCoupon(orderBy));
}

//쿠폰 상세 페이지 - 단일 쿠폰 조회
export function useGetCoupon(couponId: number) {
  return useQuery(["getCoupon"], () => getCoupon(couponId));
}

//브랜드 해당 쿠폰 리스트
export function useGetCouponList(brandName: string) {
  return useQuery(["getCouponList"], () => getCouponList(brandName));
}

//키워드 검색 쿠폰 리스트
export function useGetSearchCoupon(searchKeyword: string) {
  return useQuery(["getSearchCoupon"], () => getSearchCoupon(searchKeyword));
}
